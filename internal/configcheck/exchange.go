...
func CheckExchangePlans(proj *project.Project) []Issue {
	...
	for _, plan := range proj.ExchangePlans {
		...
		// Текущий формат использует скалярную версию объекта и безопасен только
		// для прямого обмена пары баз. Многоузловой транзит требует origin/vector
		// revision и будет отдельной версией протокола.
		if len(plan.Nodes) != 2 {
			add(plan.Name, "план обмена должен содержать ровно два узла (nodes); многоузловой транзит пока не поддерживается")
		}
