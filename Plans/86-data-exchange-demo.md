...
- Изменения регистрируются на пути UI/REST (`entityservice.Save`) и при пометке на
  удаление. **Прямые записи из DSL** (`Справочники.X.Создать().Записать()`,
  `Документы.X.Записать()`) пока не регистрируются — доработка фазы 2.
- Нет UI-монитора обмена; правило `hook` (`ПриКонфликтеОбмена`) откатывается к
  `by_time`; хаб-транзит и перенос движений — фаза 2.
